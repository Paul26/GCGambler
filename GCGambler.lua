...
function GCGambler_OnEvent()
	if ((event == "CHAT_MSG_RAID_LEADER" or event == "CHAT_MSG_RAID")and AcceptOnes=="true") then
		if (tonumber(tostring(arg1),10) == 1) then
			...
			--DEFAULT_CHAT_FRAME:AddMessage(string.format("%s%s%s","[GCG] ", tostring(arg2), " Pressed 1"));
		end
	end
...
function GCGambler_Report()
	...
	lowname = string.gsub(lowname, "^%l", string.upper)
	highname = string.gsub(highname, "^%l", string.upper)
	local string3 = "[GCG] ".. lowname.. " owes ".. highname.." ".. goldowed.." gold."
...
function GCGambler_ParseRoll(temp2)
	...
	local pattern = "^(.+) (.+) (.+) (.+)"
	local _,_,player, junk, roll, range = string.find(temp1, pattern)
	--DEFAULT_CHAT_FRAME:AddMessage( "player: "..player.." junk: "..junk.." roll: "..roll.." range: "..range );
	if junk == "rolls" and GCGambler_Check(player)==1 then
		pattern = "(.+)-(.+)"
		local _, _, minRoll, maxRoll = string.find(range, pattern)
...
function GCGambler_List()
	local string3 = "[GCG]"
	for i=1, table.getn(GCGambler.strings) do
	  	string3 = string3 .. " " .. GCGambler.strings[i]
	end
	string3 = string3 .. " still needs to roll."
	SendChatMessage(string3,"RAID",GetDefaultLanguage("player"));
end
...
function GCGambler_Add(name)
	if (name ~= nil or name ~= "") then
		...
		for i=1, table.getn(GCGambler.strings) do
			...
			found = 1;
			--DEFAULT_CHAT_FRAME:AddMessage("[GCG] ".. tostring(name).." was already found in the database.");
			end
        	end
		if found == 0 then
		      	table.insert(GCGambler.strings, name)
			totalrolls = totalrolls+1
			--DEFAULT_CHAT_FRAME:AddMessage("[GCG] ".. GCGambler.strings[table.getn(GCGambler.strings)].." has been added to the database.");
		end
	end
end
	...
	for i=1, table.getn(GCGambler.strings) do
		if GCGambler.strings[i] ~= nil then
		  	if strlower(GCGambler.strings[i]) == name then
				...
				--DEFAULT_CHAT_FRAME:AddMessage("[GCG] ".. tostring(name).." has been removed from database.");
			end
		end
      end
end
